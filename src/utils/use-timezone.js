...
export async function getTimezone(launchSiteId) {
	...
	const url = `https://api.timezonedb.com/v2.1/get-time-zone?key=${key}&format=json&by=position&lat=${latitude}&lng=${longitude}`;
...
export function useTimezone(launchSiteId) {
	return useSWR(launchSiteId, getTimezone, { loadingTimeout: 5000 });
}
